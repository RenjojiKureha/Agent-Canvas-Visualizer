...
## 目录结构
- `apps/server`: Node.js + TypeScript SSE 服务端（支持 API / Claude CLI 两种后端）
...
### 方案 A：Claude Code 本地终端（推荐本地联调）
- `AGENT_PROVIDER=claude`
- `AGENT_COMMAND=claude`

### 方案 B：OpenAI-like API（DeepSeek/OpenAI）
- `AGENT_PROVIDER=api`
- `AGENT_API_KEY=你的密钥`
...
3. 启动前端
   - `npm run dev:web`
4. 在页面输入 prompt，点击 `Start Agent Run`。
